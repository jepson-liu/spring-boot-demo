...
public class DruidDataSourceTestCase {
    ...
    @Test
    public void testDataSourceExists2() throws Exception {
        this.context.register(DruidDataSourceConfiguration.class);
        this.context.refresh();
        EnvironmentTestUtils.addEnvironment(this.context, PREFIX + "url=jdbc:mysql://172.16.1.57:3306/rbac?useUnicode=true&characterEncoding=UTF-8&useSSL=false&serverTimezone=UTC");
        EnvironmentTestUtils.addEnvironment(this.context, PREFIX + "password=test");

        EnvironmentTestUtils.addEnvironment(this.context, PREFIX + "username=test");
